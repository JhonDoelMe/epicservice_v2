...
class Product(Base):
    ...
    def група(self) -> str:
        """
        Назва групи не зберігається у таблиці ``products``. Метод повертає
        порожній рядок, щоб уникнути помилок при доступі до цього поля.
        """
        return ""

    @група.setter
    def група(self, value: str) -> None:
        """Пропускає встановлення назви групи, оскільки це поле не зберігається."""
        # Просто ігноруємо будь-яке встановлення
        pass
